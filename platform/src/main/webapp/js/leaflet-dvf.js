...
L.BarChartMarker = L.ChartMarker.extend({
	...
	_loadComponents: function () {
		...
		for (var key in data) {
			...
			options.fillOpacity = this.options.fillOpacity || 1;
...
L.DataLayer = L.LayerGroup.extend({
	...
	_bindMouseEvents: function (layer, layerOptions, legendDetails) {
		...
		var highlight = function (e) {
			...
			//todo 源码修改把tooltip中显示的坐标去掉
            var obj=parseDom(tooltip.options.icon.options.html);
            obj[0].getElementsByClassName("title")[0].remove();
            tooltip.options.icon.options.html=obj[0].innerHTML;
